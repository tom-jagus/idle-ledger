...
On startup:
- Load today’s JSON if exists
- If last block has missing/invalid `end`, treat it as open and close/reopen appropriately
- Continue tracking without losing previously stored blocks
...
- Write a line on each transition: timestamp, state change, idle, locked, inhibited, reason.
...
Summaries are generated from daily JSON files, never from running counters.

- Weekly summary: previous ISO week (Mon–Sun)
- Monthly summary: previous calendar month

Outputs:
- `summaries/week-YYYY-WW.json`
- `summaries/month-YYYY-MM.json`

Generated by CLI commands:
- `idle-ledger summarize week --week YYYY-WW`
- `idle-ledger summarize month --month YYYY-MM`
