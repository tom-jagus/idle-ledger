...
def main():
    ...
    try:
        while True:
            ...
            print(f"idle_seconds: {snapshot.idle_seconds}")
            print(f"locked: {snapshot.locked}")
            print(f"inhibited: {snapshot.inhibited}")
            print(f"state: {new_state.value}")

            totals = block_manager.get_totals()
            print(f"totals: activity={totals.activity_seconds}s, break={totals.break_seconds}s")
            print(f"timestamp: {snapshot.now_wall.isoformat()}")
            print("-" * 50)
